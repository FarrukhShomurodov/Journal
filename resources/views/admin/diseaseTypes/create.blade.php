...
@section('title')
    <title>{{ 'Findz - ' . __('commands.create') }}</title>
@endsection

@section('content')
    <h6 class="py-3 breadcrumb-wrapper mb-4">
        <span class="text-muted fw-light"><a class="text-muted" href="{{ route('diseaseTypes.index') }}">{{ __('diseaseType.diseaseTypes') }}</a> /</span>@lang('commands.create')
    </h6>
    @if ($errors->any())
    ...
    <div class="card mb-4">
        ...
        <div class="card-body">
            <form action="{{ route('diseaseTypes.store') }}" method="POST">
                @csrf
                <div class="mb-3">
